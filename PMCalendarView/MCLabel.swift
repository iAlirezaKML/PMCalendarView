...
class MCLabel: UILabel {
    ...
    var columnsSpacing: CGFloat = 1 { didSet { updateText() } }
...
//		paragraph.lineBreakMode = lineBreakMode
        ...
        attributedText.enumerateAttributesInRange(NSRange(location: 0, length: attributedText.length), options: []) { (attrs, range, stop) in
            ...
            if range.location == 0 {
                if let paragraph = attrs[NSParagraphStyleAttributeName] as? NSParagraphStyle {
                    let newParagraph = paragraph.mutableCopy() as! NSMutableParagraphStyle
                    // Same as above
                    newParagraph.lineBreakMode = .ByTruncatingMiddle
        ...
        let singlecolumnContainer = singleColumnManager.textContainers[0] 
        ...
        let columnHeight: CGFloat = frame.height
...
//            columnHeight += rect.height
        ...
        while multicolumnManager.textContainers.count != Int(numberOfColumns) {
            if multicolumnManager.textContainers.count < Int(numberOfColumns) {
                let container = NSTextContainer(size: CGSizeZero)
        ...
        for container in multicolumnManager.textContainers {
    ...
    override func drawRect(rect: CGRect) {
        let columnWidth = (frame.width - CGFloat(max(0, Int(numberOfColumns) - 1)) * columnsSpacing) / CGFloat(numberOfColumns)
        for (index, container) in multicolumnManager.textContainers.enumerate() {
