//
//  PMCalendarMonthContentView.swift
//  PMCalendar
//
...
protocol PMCalendarMonthContentViewDelegate {
...
class PMCalendarMonthContentView: UIScrollView {
	...
	var monthContentViewDelegate:PMCalendarMonthContentViewDelegate?
	
	var presentMonth         : NSDateComponents!
	var monthViewsDict   = Dictionary<String, PMCalendarMonthView>()
	var monthViews    : [PMCalendarMonthView] = []
	
	var dayViewHeight       : CGFloat             = 44
	var pastEnabled                               = false
	var presentMonthIndex   : Int!                = 0
	var selectionRangeLength: Int!                = 0
	var selectedDayViews    : [PMCalendarDayView] = []
	...
	var maxMonths           : Int! = 0 {
		didSet {
			if maxMonths > 0 {
				...
				let month = date.PMCalendarView_monthWithCalendar(presentMonth.calendar!)
				maxMonth = month
			}
		}
	}
	...
	var currentMonthView: PMCalendarMonthView! {
...
// MARK: - Navigation
extension PMCalendarMonthContentView {
	func nextMonth() {
...
extension PMCalendarMonthContentView {
	...
	func appendMonthIfNeeded() {
		if currentPage >= monthViews.count - 3 {
			...
			newMonth.month += 1
			createMonthViewForMonth(newMonth.date!.PMCalendarView_monthWithCalendar(newMonth.calendar!))
		}
	}
...
extension PMCalendarMonthContentView {
	...
	func cachedOrCreateMonthViewForMonth(month: NSDateComponents) -> PMCalendarMonthView {
		let month = month.calendar?.components(unitFlags, fromDate: month.date!)
		let monthViewKey = monthViewKeyForMonth(month!)
		var monthView = monthViewsDict[monthViewKey]
		
		if monthView == nil {
			monthView = PMCalendarMonthView(month: month!, width: bounds.width, height: bounds.height, locale: locale)
...
extension PMCalendarMonthContentView: UIScrollViewDelegate {
...
// MARK: - PMCalendarMonthViewDelegate
extension PMCalendarMonthContentView: PMCalendarMonthViewDelegate {
	func didSelectDay(dayView: PMCalendarDayView, notifyDelegate: Bool) {
		if selectionRangeLength > 0 {
			...
			for _ in 0..<selectionRangeLength {
				day = day.date!.PMCalendarView_dayWithCalendar(day.calendar!)
				let month = day.date!.PMCalendarView_monthWithCalendar(day.calendar!)
				let monthViewKey = monthViewKeyForMonth(month)
				let monthView = monthViewsDict[monthViewKey]
				let dayView = monthView?.dayViewForDay(day)
				
				if let uPMrappedDayView = dayView {
					selectDay(uPMrappedDayView)
				}
